...
판정 후 실제 API를 띄우는 명령은 [GPU 구성별 서빙 스크립트 인덱스](serving/README.md)에서 연결합니다.
...
- coding agent: MiniMax-M2.7 공식 FP8, TP=4
- 범용·멀티모달: Qwen3.5-397B-A17B INT4, TP=4
- 더 높은 정밀도: Qwen3-235B-A22B FP8

DeepSeek V4 Flash는 파일 크기만 보면 4장에 들어가지만 H100의 원본 FP4 checkpoint는 W4A16 Hopper 경로를 사용합니다. SGLang 공식 검증 구성은 H100 TP=8이므로 4×H100 추천에서 제외합니다.

### 6×H100

추천:

- 장문 context·코딩: GLM-5.2 W4A8, 잔여 약 80GB — TP=6 지원 확인 필수(미지원 시 TP=2×PP=3)
- KV 여유 운영: MiniMax-M2.7 FP8 TP=2 복제본 3개
- 경계: Qwen3.5-397B FP8(406.2GB)은 잔여 약 74GB로 긴 context 비권장

6장 구성은 TP=6을 공식 지원하는 모델이 제한적입니다. head 수·expert 수가 6으로 나눠지는지 모델 카드에서 확인하고, 안 되면 4장+2장으로 나눠 쓰는 편이 안전합니다.
...
| 코딩·1M context·멀티모달 | [MiniMax-M3](https://huggingface.co/MiniMaxAI/MiniMax-M3) | 약 430GB, FP8 | TP=8, 잔여 ~200GB로 KV 확장 | SGLang/vLLM |
...
따라서 8×H100의 기본 1순위는 하나가 아닙니다. 장문 context는 GLM-5.2 W4A8, Kimi 계열 agent 품질은 K2.6/K2.7 INT4 경계 구성, 최신 대형 MoE는 DeepSeek V4 Flash TP=8, 안정적인 서비스는 Qwen3.5-397B FP8, coding agent 총 처리량은 MiniMax-M2.7 TP=4 복제본 두 개가 우선입니다. 1M context·멀티모달 입력까지 한 모델로 필요하면 MiniMax-M3 FP8이 후보입니다.
